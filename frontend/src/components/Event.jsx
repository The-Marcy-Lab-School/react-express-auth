...
import { joinAnEvent, leaveAnEvent, fetchAttendeesAmount } from "../adapters/event-adapter";
...
const Event = (props) => {
    ...
    const [attendeeAmount, setAttendeeAmount] = useState(0)
        const formattedStartDate = new Date(event.date).toLocaleString('en-US',timeObject)
        const formattedEndDate = new Date(event.end_date).toLocaleString('en-US',timeObject)
  ...
  return (
    ...
    {event.id && <JoinButton joinEvent={joinEvent} eventId={event.id} joinedEvents={joinedEvents}/>} {/*ill see if I need the && later*/}
